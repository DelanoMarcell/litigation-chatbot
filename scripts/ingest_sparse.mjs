...
const MIN_WORDS = 350;
const MAX_WORDS = 800;
const UPSERT_BATCH = Number(process.env.SPARSE_UPSERT_BATCH || 64);
const PINECONE_NAMESPACE = process.env.PINECONE_NAMESPACE || "default";
const UPSERT_DELAY_MS = Number(process.env.SPARSE_UPSERT_DELAY_MS || 0);
const MAX_RETRIES = Number(process.env.SPARSE_UPSERT_MAX_RETRIES || 6);
const BACKOFF_BASE_MS = Number(process.env.SPARSE_UPSERT_BACKOFF_MS || 1500);
const BACKOFF_MAX_MS = Number(process.env.SPARSE_UPSERT_BACKOFF_MAX_MS || 60000);
...
async function run() {
  ...
  console.log(`Prepared ${allChunks.length} chunks. Upserting to sparse index in batches of ${UPSERT_BATCH}.`);
  ...
  for (let i = 0; i < allChunks.length; i += UPSERT_BATCH) {
    ...
    while (true) {
      try {
        await index.namespace(PINECONE_NAMESPACE).upsertRecords(records);
        console.log(`Upserted ${i + batch.length}/${allChunks.length}`);
        break;
      } catch (err) {
        attempt += 1;
        const message = String(err?.message || err);
        const isRateLimit = /RESOURCE_EXHAUSTED|429|rate limit|tokens per minute/i.test(message);
        ...
        console.warn(`Rate limited. Retrying in ${backoff}ms (attempt ${attempt}/${MAX_RETRIES})`);
        await sleep(backoff);
      }
    }
  ...
  console.log("Sparse ingestion complete.");
}

run().catch((err) => {
  console.error("Sparse ingestion failed:", err.message);
  process.exit(1);
});
